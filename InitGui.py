"""PartDesignTimeline — a Fusion-360-style history scrubber for FreeCAD.
...
- a draggable SCRUBBER slider: drag it and the model rebuilds to that point in
  time (true "travel in time" — shows the solid state at that step, with the
  pointed-at sketch/datum overlaid)
- click a chip to jump the scrubber there; double-click a chip to edit that feature

Per active Body (FreeCAD's model of linear history). Maps onto Body.Tip +
feature visibility. 
"""
...
class TimelineWidget(QtWidgets.QWidget):
    ...
    def _context_menu(self, btn):
        feat = getattr(btn, '_tl_feat', None)
        if feat is None:
            return
        m = QtWidgets.QMenu()
        ...
        act = m.exec_(btn.mapToGlobal(btn.rect().bottomLeft()))
        try:
            if act == a_rename:
                self._rename(feat)
            elif act == a_edit:
    ...
    def _rename(self, feat):
        try:
            new, ok = QtWidgets.QInputDialog.getText(
                self, "Rename feature", "New name:", text=feat.Label)
            if ok and new:
                feat.Label = new
                self.refresh()
        except Exception as e:
    ...
    def eventFilter(self, obj, ev):
        if hasattr(obj, '_tl_feat'):
            if ev.type() == QtCore.QEvent.MouseButtonDblClick:
                try:
                    Gui.activeDocument().setEdit(obj._tl_feat)
                except Exception as e:
...
QtCore.QTimer.singleShot(1500, install_timeline)
